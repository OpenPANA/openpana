...
SM_STATE(EAP, GET_METHOD)
{
	...
	if (sm->m == NULL)
		sm->m = eap_peer_get_eap_method(sm->eap_methods,sm->reqVendor, method);
	if (!sm->m) {
				  ...
				  EapType method)
{
	...
	if (eap_peer_get_eap_method(sm->eap_methods,vendor, method))
	{
		return TRUE;
	}
...
static struct wpabuf * eap_sm_buildNak(struct eap_sm *sm, int id)
{
	...
	methods = eap_peer_get_methods(sm->eap_methods,&count);
	if (methods == NULL)
		return NULL;
	if (sm->reqMethod == EAP_TYPE_EXPANDED)
				 ...
				 void *msg_ctx, struct eap_config *conf)
{
	...
	sm->eap_methods = conf->eap_methods;
 ...
 */
u32 eap_get_phase2_type(struct eap_sm *sm, const char *name, int *vendor)
{
	int v;
	u8 type = eap_peer_get_type(sm->eap_methods,name, &v);
 ...
 */
struct eap_method_type * eap_get_phase2_types(struct eap_sm *sm, struct eap_peer_config *config,
					      size_t *count)
{
	...
	methods = eap_peer_get_methods(sm->eap_methods,&mcount);
	if (methods == NULL)
		return NULL;
	*count = 0;
