...
static void * eap_ttls_init(struct eap_sm *sm)
{
	...
	if (data->phase2_type == EAP_TTLS_PHASE2_EAP) {
		if (eap_peer_select_phase2_methods(sm,config, "autheap=",
					      ...
					      u8 method, struct wpabuf **resp)
{
	...
	if (data->phase2_priv == NULL) {
		data->phase2_method = eap_peer_get_eap_method(sm->eap_methods,
...
int eap_peer_ttls_register(struct eap_method **eap_methods)
{
	...
	ret = eap_peer_method_register(eap_methods,eap);
	if (ret)
