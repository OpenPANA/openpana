 ...
 */
int eap_peer_select_phase2_methods(struct eap_sm *sm, struct eap_peer_config *config,
				   const char *prefix,
				   ...
				   size_t *num_types)
{
	...
	while (start && *start != '\0') {
		...
		if (pos)
			*pos++ = '\0';
		method = eap_get_phase2_type(sm, start, &vendor);
...
get_defaults:
	if (methods == NULL)
		methods = eap_get_phase2_types(sm, config, &num_methods);
