...
int eap_peer_init(struct eap_peer_ctx *eap_ctx, void *eap_ll_ctx)
{
	...
	eap_ctx->eap_config.identity = (u8 *) os_strdup("pepe");
	eap_ctx->eap_config.identity_len = 4;
	eap_ctx->eap_config.password = (u8 *) os_strdup("holapepe");
	eap_ctx->eap_config.password_len = 8;
	eap_ctx->eap_config.client_cert = (u8*) os_strdup("client0.pem");
	eap_ctx->eap_config.private_key=(u8*) os_strdup("client0.pk8");
	eap_ctx->eap_config.ca_cert = (u8 *) os_strdup("ca.pem");
	eap_ctx->eap_config.private_key_passwd = (u8*) os_strdup("josejose");
